...
!    This file is part of STATAMOD. Copyright (c) 2006-2010 Andrew Shephard
!
...
contains     
    ...
    subroutine saveStata(fileName,thisUnit,obs,label)
        ...
        open(stataFile%saveUnit,FILE=fileName,ACTION='write',STATUS='REPLACE',FORM='BINARY',IOSTAT=ios)
    ...
    subroutine openStata(fileName,thisUnit,cache)
        ...
        statafile%unit = thisUnit
        
        open(stataFile%unit,FILE=fileName,action='read',status='old',form='binary',iostat=ios)
        ...
        do while (tempInt1 /= 0 .and. tempInt4 /=0)
            read(stataFile%unit) tempInt1
            read(stataFile%unit) tempInt4
            filePointer = fseek(stataFile%unit,tempInt4,1)
        end do

        !position of start of data
        
        stataFile%stataPosition%start = ftell(stataFile%unit)
